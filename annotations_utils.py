...
def collect_pano_ids(srcs: List[str], exclude_prefix: str = None) -> Tuple[List[str], List[str]]:
    """
    ...
    return filenames_TMX_as_list, ids_TMX_as_list


def get_filenames_metadata(filenames: List[str], ids: List[str]) -> Container[PointOfInterest]:
    """

    return map with images_coords
    """
    filenames_loc = Container[PointOfInterest]()
    for filename, img_id in tqdm(zip(filenames, ids), total=len(filenames), desc="Getting metadata from API"):
