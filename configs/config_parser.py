...
def arg_parser() -> argparse.Namespace:
    ...
    if flags.inference is True and not flags.version:
        parser.error(f"Specify which version of {flags.name} model to load by using --version.")
    if flags.train is True and flags.subset != "train":
        parser.error("Training model should happen on the training subset. Set --subset to train.")
