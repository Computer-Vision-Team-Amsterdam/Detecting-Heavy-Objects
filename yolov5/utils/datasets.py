...
from utils.general import clean_str, xywh2xyxy, xywhn2xyxy, xyxy2xywh
...
def create_dataloader(
    ...
    nw = 0  # Todo: WHhy is is this not working?
    sampler = (
    ...
    dataloader = loader(
        ...
        sampler=sampler,
        pin_memory=True,
        collate_fn=LoadImagesWithoutLabels.collate_fn4
        if quad
        else LoadImagesWithoutLabels.collate_fn,
    )
...
def img2label_paths(img_paths):
    ...
    return [
        x.replace(sa, sb, 1).replace("." + x.split(".")[-1], ".txt") for x in img_paths
    ]


class LoadImagesWithoutLabels(Dataset):
    def __init__(
        self,
        path,
        img_size=640,
        ...
        self.path = path
        try:
            # panorama's are always jpg files, but might be a bit hardcoded
            f = [str(file) for file in path.iterdir() if file.suffix == ".jpg"]
    ...
    def __getitem__(self, index):
        ...
        img, ratio, pad = letterbox(img, (h, w), auto=False, scaleup=self.augment)
...
def load_mosaic9(self, index):
    ...
    # Offset
    yc, xc = [
        int(random.uniform(0, s)) for x in self.mosaic_border
    ]  # mosaic center x, y
