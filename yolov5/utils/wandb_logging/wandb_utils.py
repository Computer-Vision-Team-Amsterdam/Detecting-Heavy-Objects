...
try:
    ...
    wandb = None
    print(
        f"{colorstr('wandb: ')}Install Weights & Biases for YOLOv5 logging with 'pip install wandb' (recommended)"
    )
...
class WandbLogger:
    ...
    def setup_training(self, opt, data_dict):
        ...
        (
            ...
            self.trainset_artifact,
        ) = self.download_dataset_artifact(data_dict["train"], opt.artifact_alias)
        self.test_artifact_path, self.testset_artifact = self.download_dataset_artifact(
            data_dict["val"], opt.artifact_alias
        )
        ...
        if self.wandb_run:
            if self.result_artifact:
                ...
                train_results = wandb.JoinedTable(
                    self.testset_artifact.get("val"), self.result_table, "id"
                )
